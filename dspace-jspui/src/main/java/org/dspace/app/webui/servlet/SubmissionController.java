...
public class SubmissionController extends DSpaceServlet
{
            ...
            SQLException, AuthorizeException
    {
        ...
        if(step.equals("2")){
            String coverages = request.getParameter("identifier");

            SoapHelper sh = new SoapHelper();
            Document docMeta = null;
            docMeta = sh.getRecordById(coverages);

            if(coverages != null) {


                WorkspaceItem wsi = WorkspaceItem.find(context, Integer.parseInt(itemId));
        ...
        if(item_id != null) {
            if (!item_id.equals("")) {
              ...
              //  request.setAttribute("identifier", idItem);
            }
        }
            ...
            if(stepFinished)
            {
                ...
                if(stepNumber == 1){
                    ...
                    if((iden != null) || (item_id != null)){
                        ...
                        if(item_id != null){
                            //request.setAttribute("identifier", item_id);
                        ...
                        NodeList coverage = null;
                        try {
                            coverage = (NodeList) expr.evaluate(docMeta, XPathConstants.NODESET);
                        } catch (XPathExpressionException e) {
                            e.printStackTrace();
                        }
                        writeMetaDataToItemLowerCaseSubject(ti, "subject", coverage);
                        ...
                        writeMetaDataToItemLowerCaseDescr(ti, "coverage", cover);
